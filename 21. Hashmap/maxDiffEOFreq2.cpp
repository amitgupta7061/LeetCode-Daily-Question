...
public:
    int maxDifference(string s, int k){
        int ans = INT_MIN;
        for (auto &[a, b] : getPermutations()){
            vector<vector<int>> minDiff(2, vector<int>(2, INT_MAX / 2));
            vector<int> prefixA{0}, prefixB{0};
            for (int l = 0, r = 0; r < s.length(); ++r){
                ...
                while (r - l + 1 >= k && prefixA[l] < prefixA.back() && prefixB[l] < prefixB.back()){
                    minDiff[prefixA[l] % 2][prefixB[l] % 2] = min(minDiff[prefixA[l] % 2][prefixB[l] % 2], prefixA[l] - prefixB[l]);
                    ++l;
                }
                ans = max(ans, (prefixA.back() - prefixB.back()) - minDiff[1 - prefixA.back() % 2][prefixB.back() % 2]);
            }
        }
        return ans;
    }
...
private:
    vector<pair<char, char>> getPermutations(){
        ...
        for (char a : "01234")
            for (char b : "01234")
                if (a != b) permutations.emplace_back(a, b);
        return permutations;
    }
};
